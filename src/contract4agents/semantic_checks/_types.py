...
def _named_type_names(type_ref: TypeRef) -> set[str]:
    ...
    if isinstance(type_ref, PrimitiveTypeRef | ConstrainedTypeRef):
        return set()
...
def _check_enum_default(
    ...
    if _default_conforms(value, type_ref, index):
        return []
    return [
        Diagnostic(
            "SEM017",
    ...
    if isinstance(type_ref, ConstrainedTypeRef):
        if type_ref.item.name == "string":
            if not isinstance(value, str):
                return False
            return (
                (type_ref.min_length is None or len(value) >= type_ref.min_length)
                and (type_ref.max_length is None or len(value) <= type_ref.max_length)
            )
        if isinstance(value, bool) or not isinstance(value, int | float):
            return False
        if type_ref.item.name == "integer" and not isinstance(value, int):
            return False
        return (
            (type_ref.minimum is None or value >= type_ref.minimum)
