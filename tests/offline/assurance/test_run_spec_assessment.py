...
def _materialization_evidence(
    ...
    return GraphValidationEvidence(
        ...
        schema_conformance=tuple(
            SchemaConformanceEvidence(agent_id, "agent_output", schema, schema) for agent_id in ir.agents
        ),
...
def _closure(trace: NormalizedTrace) -> TraceClosureEvidence:
    attempts = {TraceAttempt.from_dict(event.data["attempt"]): event.semantic.agent_id for event in trace.events}
