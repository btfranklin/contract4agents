...
def test_assurance_bundle_is_deterministic_verified_and_explicit_about_missing_evidence(
    ...
    for supplied in ((), results[:1]):
        ...
        diagnostic = next(item for item in incomplete_inventory.diagnostics if item.code == "BUNDLE018")
        missing = sorted(
            str(item.id)
            for item in ir.controls.values()
            if str(item.id) not in {result.control_id for result in supplied}
        )
...
def _materialization_evidence(ir: CanonicalIR, plan: MaterializationPlan) -> GraphValidationEvidence:
    ...
    checks = [SchemaConformanceEvidence(agent_id, "agent_output", schema, schema) for agent_id in ir.agents]
    checks.extend(
