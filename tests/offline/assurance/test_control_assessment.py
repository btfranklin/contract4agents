...
def test_control_assessor_approval_failure_and_missing_evidence_branches(
    ...
    ir = campaign_ir()
    plan = campaign_plan(ir, expected_event_types=("event.never_emitted",))
    attempt = TraceAttempt("support:1", "support-attempt-1", 1)
    trace_events = tuple(
        _event(
            f"evt-{index}",
...
def test_conditional_control_distinguishes_false_true_and_unverified_applicability() -> None:
    ...
    plan = replace(
        base_plan,
        contract_digest=contract_digest(ir),
        controls=FrozenMap({control.id: ControlMappingPlan(control.id, True, "post_run", "exact", "test", ())}),
...
def test_control_assessor_prefers_explicit_results_and_handles_output_failures() -> None:
    base = campaign_ir()
    plan = campaign_plan(base, expected_event_types=("control.assessed",))
    ...
    plan = replace(
        plan,
        contract_digest=contract_digest(ir),
        controls=FrozenMap({custom_id: ControlMappingPlan(custom_id, False, "runtime", "exact", "test", ())}),
    )
    trace_events = (
        _event(
            "evt-1",
