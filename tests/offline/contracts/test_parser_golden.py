...
def _snapshot(project: object) -> dict[str, list[str]]:
    ...
    return {
        ...
        "external_contexts": sorted(
            ...
            for item in project.external_contexts.values()  # type: ignore[attr-defined]
        ),
        "grants": sorted(
        ...
        "operational_controls": sorted(
            f"{item.agent}.{item.name}"
            for item in project.operational_controls  # type: ignore[attr-defined]
        ),
