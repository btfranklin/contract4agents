...
def test_conformance_rejects_a_loaded_host_module_without_replacing_it(tmp_path: Path) -> None:
    ...
    before = {
        name: module for name, module in sys.modules.items() if name == "calendar" or name.startswith("calendar.")
    }
...
def test_conformance_requires_complete_profiles_without_unknown_agent_overrides(tmp_path: Path) -> None:
    bindings = _bindings(
        ...
        external_context={"incident_record": None},
        profiles={
            "incomplete": TargetProfile(agents={"RemovedAgent": AgentProfile(model="stale-model")}),
...
def test_openai_conformance_rejects_ambiguous_or_unsupported_locators(
    ...
    target = TargetBinding(
        ...
        tools={
            "incident.fetch_logs": (
                entry if section == "tools" else BindingEntry({"python": f"{MODULE_NAME}:fetch_logs"})
            )
        },
        datasources={
            "incident.timeline": (
                entry if section == "datasources" else BindingEntry({"python": f"{MODULE_NAME}:timeline"})
            )
        },
        external_context={
            "incident_record": (
                entry if section == "external_context" else BindingEntry({"python": f"{MODULE_NAME}:current_incident"})
            )
        },
...
def test_openai_conformance_accepts_canonical_web_search_binding(tmp_path: Path) -> None:
    ...
    target = TargetBinding(
        adapter="openai",
        tools={"incident.fetch_logs": BindingEntry({"provider": "openai", "tool": "web_search"})},
        datasources={"incident.timeline": BindingEntry({"python": f"{MODULE_NAME}:timeline"})},
        external_context={"incident_record": BindingEntry({"python": f"{MODULE_NAME}:current_incident"})},
        profiles={"test": TargetProfile(default_model="test-model")},
    )
    bindings = TargetBindings(
        tmp_path / "contract4agents.targets.toml",
