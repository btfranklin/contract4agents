...
def test_plan_serialization_defensively_rejects_credential_keys() -> None:
    ...
    unsafe_plan = replace(
        plan,
        agents=FrozenMap(
            (identifier, unsafe_agent if identifier == agent_id else agent) for identifier, agent in plan.agents.items()
        ),
    )
...
def test_agent_model_options_override_profile_options_and_affect_plan_digest() -> None:
    ...
    assert (
        commander.model_options["model_factory"] == investigator.model_options["model_factory"] == "app_models:create"
    )
...
def test_supported_single_run_operational_control_has_required_evidence() -> None:
    ...
    assert any(
        item.code == "host.provide_operational_evidence" and item.semantic_id == control_id
...
def test_required_degraded_control_blocks_while_advisory_unsupported_is_visible() -> None:
    ...
    degraded = _capabilities(controls={"adapter": MappingSupport("degraded", "best_effort.adapter")})
    with pytest.raises(PlanningError) as caught:
        plan_materialization(required, _bindings(), target="openai", profile="production", capabilities=degraded)
    assert any(
        item.semantic_id == semantic_id("control", "IncidentCommander", "runtime_check") for item in caught.value.issues
    )
...
def test_legacy_mapping_resolver_without_contextual_hooks_still_plans() -> None:
    ...
    assert plan.bindings[semantic_id("tool", "status.publish")].outcome == "exact"
...
def test_openai_planner_rejects_unsupported_tool_binding_shapes(
    ...
    assert any(
        issue.code == "PLN009" and issue.semantic_id == semantic_id("tool", "status.publish")
        for issue in caught.value.issues
    )
...
def test_openai_planner_rejects_provider_hosted_datasource() -> None:
    ...
    rebound = TargetBinding(
        adapter=target.adapter,
        tools=target.tools,
        datasources={"incident.timeline": BindingEntry({"provider": "openai", "tool": "web_search"})},
        external_context=target.external_context,
        environments=target.environments,
        profiles=target.profiles,
    )
    ...
    assert any(
        issue.code == "PLN009" and issue.semantic_id == semantic_id("datasource", "incident.timeline")
        for issue in caught.value.issues
    )
...
def test_openai_rejects_named_tool_execution_environment() -> None:
    ...
    assert any(
        issue.code == "PLN009" and issue.semantic_id == semantic_id("grant", "IncidentCommander", "status.publish")
        for issue in caught.value.issues
    )
...
def _openai_hosted_bindings() -> TargetBindings:
    ...
    rebound = TargetBinding(
        adapter=target.adapter,
        tools={"status.publish": BindingEntry({"provider": "openai", "tool": "web_search"})},
        datasources=target.datasources,
        external_context=target.external_context,
        environments=target.environments,
        profiles=target.profiles,
...
def _openai_ir(
    *,
    ...
    return CanonicalIR.create(
        ...
        isolation_profiles=(base.isolation_profiles.values() if grant_isolation or edge_isolation else ()),
    )
