...
def test_google_search_binding_is_schema_aware_and_emulated(tmp_path: Path) -> None:
    ...
    output = plan.controls[semantic_id("control", "Researcher", "output_conformance")]
    assert output.outcome == "emulated"
    assert any("display Google Search suggestions" in obligation.description for obligation in plan.host_obligations)
...
def test_google_adk_profile_requires_gemini_without_factory(tmp_path: Path) -> None:
    ...
    target = TargetBinding(
        ...
        profiles={
            "test": TargetProfile(
                default_model="claude-provider-model",
                agents={"Researcher": AgentProfile(model="other-provider-model")},
            )
        },
    )
