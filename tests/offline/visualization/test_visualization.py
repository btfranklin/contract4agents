...
def test_declared_graph_preserves_enum_values_as_type_metadata() -> None:
    ir = CanonicalIR.create(types=(EnumIR(semantic_id("type", "Status"), "Status", ("accepted", "failed")),))
...
def test_graph_keeps_declared_planned_observed_and_assured_truth_separate() -> None:
    ...
    assert [(edge["source"], edge["target"]) for edge in evidence_edges] == [(str(control_id), "event:event-1")]
...
def test_presentation_model_is_deterministic_and_domain_specific() -> None:
    ...
    assert {item["id"] for item in presentation["overview_agents"]} == {f"agent:{name}" for name in graph["agents"]}
    assert {item["source"] for item in presentation["overview_relationships"]} == {"agent:IncidentCommander"}
...
def test_presentation_surfaces_unsupported_target_controls() -> None:
    ...
    presentation = build_visualization_presentation(graph)
    rendered = next(
        item for focus in presentation["focus"].values() for item in focus["controls"] if item["id"] == control["id"]
    )
