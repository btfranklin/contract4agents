...
def test_definitions_references_and_rename_are_project_wide(tmp_path: Path) -> None:
    ...
    assert service.definition(agent_path, input_position) == [(types_path, _range_of(TYPES, "SectionResearchInput"))]
    assert service.definition(agent_path, tool_position) == [(types_path, _range_of(TYPES, "research.web_search"))]
...
def test_full_language_surface_is_indexed_and_rendered(tmp_path: Path) -> None:
    fixture_root = Path(__file__).parents[3] / "editors" / "vscode" / "test" / "fixtures"
