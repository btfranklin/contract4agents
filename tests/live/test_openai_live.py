...
PROMPT = ROOT / "tests" / "fixtures" / "prompts" / "incident-live.md"
...
async def test_contract_first_incident_graph_runs_through_openai(
    ...
    system = materialize(
        ...
        target="openai",
        profile="production",
        normalized_trace_sink=runtime_events,
    )
    ...
    completed_agents = {event.semantic.agent_id for event in trace.events if event.event_type == "agent.completed"}
...
async def test_openai_hosted_web_search_normalizes_to_exact_grant() -> None:
    ...
    hosted_events = [
        ...
        if event.event_type == "tool.completed" and event.semantic.capability_id == semantic_id("tool", "web.search")
    ]

    assert len(hosted_events) == 1
    assert hosted_events[0].semantic.grant_id == semantic_id("grant", "CurrentTruthScout", "web.search")
