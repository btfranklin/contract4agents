...
async def test_contract_first_incident_graph_runs_through_strands_bedrock(
    ...
    if os.environ.get("CONTRACT4AGENTS_RUN_STRANDS_LIVE") != "1":
        pytest.skip("set CONTRACT4AGENTS_RUN_STRANDS_LIVE=1 to run the live Strands smoke test")
    ...
    completed_agents = {
        event.semantic.agent_id for event in snapshot.trace.events if event.event_type == "agent.completed"
    }
    assert {
    ...
    assert any(event.event_type == "composition.completed" for event in snapshot.trace.events)
